  ...
  return (
    <div className="card">
      <div className="our-card-top">
        ...
        <img src={props.photo ? `/upload-photos/${props.photo}` : "/fallback.png"} className="card-img-top" alt={`${props.species} named ${props.name}`} />
      </div>
          ...
          <>
            ...
            {!props.readOnly && (
              <>
                ...
                <button
                  onClick={async () => {
                    const test =await Axios.delete(`/animal/${props.id}`)
                    props.setAnimals(prev => {
